...
function App() {
  return <OTPInput length={6} onComplete={(code) => console.log('OTP Verified:', code)} />;
}
```
...
We ignore where you are focused when you paste. If you paste a code, we assume it's the _whole_ code. We always distribute the pasted value from index 0, ensuring your users never end up with shifted or corrupted inputs.
